...
export default function SupplierClientPage({ supplierId }: { supplierId: string }) {
  const { isLoading, error, products } = useSupplierData(supplierId);
  const { supplierName } = useSupplierTheme(supplierId);

  return (
    <Layout supplierId={supplierId}>
      {isLoading ? (
